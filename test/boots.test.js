...
/**
 * The bot can actually be loaded.
 *
...
const ENTRY = 'discord-bot/index.js';
...
test('the entrypoint loads, or fails only for something this machine lacks', () => {
  const r = spawnSync(
    ...
    // The require is followed immediately by process.exit(0), so client.login()'s
    // promise never settles and a missing token cannot fail this. What is being
    // detected is a LOAD-time error, which is the failure that leaves a
